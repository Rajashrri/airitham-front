...
import logodark from "../../assets/images/airithm-logo.svg";
import logolight from "../../assets/images/airithm-logo.svg";
...
const VerifyOtpPage = () => {
  ...
  const handleResendOtp = async () => {
    try {
      const email = localStorage.getItem("email");
      if (!email) return toast.error("Email not found in session.");

      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/auth/resend-login-otp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
                        ...
                        <div className="auth-form-group-custom mb-4">
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              handleChange({
                                target: { name: "otp", value: onlyNums },
                              });
                            }}
                            onBlur={handleBlur}
                            value={values.otp}
