      ...
      <Row className="mb-2">
        ...
        <Col md={6}>
          <div className="d-flex justify-content-end">
            <Link to="/add-testimonial" className="btn btn-primary">
              Add
            </Link>
          </div>
        </Col>
...
const TestimonialsList  = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/testimonial/getdatatestimonial`,
        {
          method: "GET",
        }
      );
      ...
      setTestimonialList(result.msg);
    } catch (error) {
      console.error("Error fetching data:", error);
      toast.error("Failed to load testimonial data. Please try again later.");
    }
  };
  ...
  const handleyesno = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/testimonial/deletetestimonial/${deleteId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleChange = async (currentStatus, id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/testimonial/update-statustestimonial`,
        {
          method: "PATCH",
          headers: {
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="d-flex gap-2">
            <Link  to={`/update-testimonial/${row.original._id}`}   className="btn btn-primary btn-sm">
              Edit
            </Link>
            <Button
              color="danger"
  ...
  return (
    <Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="testimonial" breadcrumbItems={breadcrumbItems} />
          <Card>
            <CardBody>
              <TableContainer
                columns={columns}
